...
public class TestHmw {
	...
	private static void Driver() {
		...
		Street_LDList kemerSokak = new Street_LDList(120);/*Constant Time*/
		kemerSokak.setRigthBuildings(new LDLinkedList<Structure>());/*Constant Time*/
		kemerSokak.setLeftBuildings(new LDLinkedList<Structure>());/*Constant Time*/
		...
		try {
			...
			for(int i=0; i<1000; i++)
				kemerSokak.getRigthBuildings().add(new House(0, 1, 14, "Elon Husky", 10, "Bed"));	
		...
		kemerSokak.displayBuildingList();				 /*O(N^2)*/
		System.out.println("##\n");
		...
		kemerSokak.DrawSilhouette();			/*O(N^3)*/
		...
		for(int i=0; i<kemerSokak.getRigthBuildings().size();++i) 	/*O(N^2)*/
			kemerSokak.getRigthBuildings().get(i).presentInfo();
		
		for(int i=0; i<kemerSokak.getLeftBuildings().size();++i)	/*O(N^2)*/
			kemerSokak.getLeftBuildings().get(i).presentInfo();
		

		/*O(N^4)*/
		System.out.println("\nCotrolling that There is a building in 10!!");
		if(!Street_LDList.isValidPlace(kemerSokak.getLeftBuildings(),10,30))
	...
	public static void Menu() {
		...
		Street_LDList kemerSokak = new Street_LDList(x);
	    kemerSokak.setRigthBuildings(new LDLinkedList<Structure>());
	    kemerSokak.setLeftBuildings(new LDLinkedList<Structure>());
			...
			while(userModeChoice==2 && !exitUserMode && !invalidOption) {
				...
				while(choice2==3 && !exitSubUserModes) {
					...
					try {
						...
						if(side==1) {
							int t = Street_LDList.findFocusingIndex(pos,kemerSokak.getRigthBuildings());
							if(t==-1)
								System.out.println("Given Position is not correct!!");
							else kemerSokak.getRigthBuildings().get(t).presentInfo();
						}
						else if(side==2) {
							int t = Street_LDList.findFocusingIndex(pos,kemerSokak.getLeftBuildings());
	...
	public static <E>void Delete(int startPos, LDLinkedList<Structure> str) throws Exception{
		

		int index = Street_LDList.findObjIndex(startPos,str);
