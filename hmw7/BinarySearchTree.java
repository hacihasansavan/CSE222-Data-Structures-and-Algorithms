...
public class BinarySearchTree < E
 ...
 /* public BinarySearchTree(BinaryTree.Node<E> root) {
  ...
  /** Recursive find method.
      ...
      @return The object, if found, otherwise null
   */
  private Node<E> find(Node < E > localRoot, int target) {
    if (localRoot == null)
      return null;
    if (localRoot.data.equals(target))
      return localRoot;
    //  else if (target < (int) localRoot.data.compareTo(target)>0)
      int temp = (int)(Object)localRoot.data;
    if (target < temp)
      return find(localRoot.left, target);
    else
      return find(localRoot.right, target);
  ...
  public void poco(Node<E> root, E target, Node<E> node) {
    if (root == null)
      return;
    ...
    if (compResult == 0){
      ...
      poco(root.left, target, node);
    else
      poco(root.right, target, node);
  }
